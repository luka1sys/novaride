...
import {
    ...
    IconLogout, IconMenu2, IconX, IconActivity, IconWallet
...
const AdminPanel = () => {
    ...
    const menuItems = [
        ...
        { id: "bookings", label: "Reservation Lab", icon: <IconCalendarStats size={20} /> },
    ];

    return (
        /* დავამატე pt-20 (დიდ ეკრანზე) და pt-24 (მობილურზე) რომ Navbar-ს დაცილდეს */
        <div className="flex h-screen bg-[#050505] font-sans text-white overflow-hidden pt-[80px] md:pt-[90px] relative z-[10]">
            ...
            <AnimatePresence>
                {isSidebarOpen && (
                    <motion.div
                        ...
                        animate={{ opacity: 1 }}
                        exit={{ opacity: 0 }}
                        // z-50 რომ Navbar-ს გადაეფაროს მენიუს გახსნისას
            ...
            <main className="flex-1 flex flex-col h-full overflow-y-auto no-scrollbar relative z-10">
                ...
                {/* დავუწიე Header-ის სიმაღლე 24-დან 20-მდე, რომ მეტი ადგილი დარჩეს */}
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h1 className="text-2xl font-black uppercase tracking-tighter italic">
                                {activeTab} <span className="text-gray-600">Control</span>
                            </h1>
                ...
                <div className="p-8 md:p-12 pt-6">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
                        {[
                            ...
                            {
                                label: "users",
                                val: users.length.toString(), // აქ ვიყენებთ users მასივის სიგრძეს
                                icon: <IconActivity />,
                                color: "rgb(249, 115, 22)"
                            },
                            { label: "System Health", val: "100%", icon: <IconSettings />, color: "#22c55e" }
                        ].map((stat, i) => (
                            <div key={i} className="bg-[#0A0A0A] border border-white/5 p-6 rounded-[32px] group hover:border-white/10 transition-all duration-500">
                                <div className="w-10 h-10 rounded-2xl bg-white/5 flex items-center justify-center mb-4 group-hover:scale-110 transition-transform" style={{ color: stat.color }}>
                                    {stat.icon}
                                </div>
                                <p className="text-gray-500 text-[10px] font-black uppercase tracking-widest">{stat.label}</p>
                                <p className="text-3xl font-black mt-1 italic">{stat.val}</p>
                            </div>
                        ))}
                    </div>
                    ...
                    <div className="relative">
                        ...
                        <div className="bg-[#0A0A0A] rounded-[40px] border border-white/5 shadow-2xl shadow-black overflow-hidden relative">
                            <div className="p-6 border-b border-white/5 flex justify-between items-center bg-white/[0.01]">
                                <h3 className="text-xs font-black uppercase tracking-[0.2em] text-gray-400">Live Data Feed</h3>
